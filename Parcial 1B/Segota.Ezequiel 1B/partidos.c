...
int getNextIdPartido(ePartido partido[], int tamanioPartido)
{
    ...
    {
        if(partido[i].isEmpty == VACIO || partido[i].isEmpty == BAJA)
        {
            i++;
            break;
        }
...
int menuListadosPartidos(void)
{
    ...
    printf("  5- Listar Referis con mas de 4 Partidos\n");
    printf("  6- Listar Partidos ordenados por Referi\n");
    printf("  7- Salir\n");
...
{
    ePartido auxPartido;
    char auxReferiApellidoDeI[64];
    char auxReferiApellidoDeJ[64];
    system("cls");
    printf("  *** Lista de Partidos ***\n");
    printf("  Ordenados por Referi(Apellido).\n\n");
    for(int i=0; i<tamanioPartido-1; i++)
    {
        for(int j=i+1; j<tamanioPartido; j++)
        {
            for(int k=0;k<tamanioReferi;k++)
            {
                if(referis[k].idReferi==partido[i].idReferi)
                {
                    cargarReferi(referis,tamanioReferi,referis[k].idReferi,auxReferiApellidoDeI);
                }
                if(referis[k].idReferi==partido[j].idReferi)
                {
                    cargarReferi(referis,tamanioReferi,referis[k].idReferi,auxReferiApellidoDeJ);
                }
            }
            if(strcmp(auxReferiApellidoDeI,auxReferiApellidoDeJ)>0)
            {
            ...
            do
            {
                switch(menuListadosPartidos())
                {
                    ...
                    case 4:
                        ...
                        break;
                    case 5:
                        listarReferisConMasDeCuatrosPartidos(partido,tamanioPartido,referis,tamanioReferi);
                        system("pause");
                        break;
                    case 6:
                        listarPartidosOrdenadosPorArbitro(partido,tamanioPartido,equipos,tamanioEquipos,referis,tamanioReferi);
                        system("pause");
                        break;
                    case 7:
                        system("cls");
